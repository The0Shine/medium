...
const postService = {
  ...
  async createPost(postData, userid) {
    ...
    if (languageid && create_for_all_languages) {
      try {
        ...
        const sessionId = `post_${post.postid}_${Date.now()}`;
        translationProgressService.startSession(sessionId, availableLanguages.length, availableLanguages);

        // Create translations for each available language
        const translationPromises = availableLanguages.map(async (language) => {
          try {
            // Update progress: start translating this language
            translationProgressService.updateLanguageProgress(sessionId, language.languageid, 'translating');
            ...
            // Update progress: translation completed
            translationProgressService.updateLanguageProgress(sessionId, language.languageid, 'completed');
            ...
            // Update progress: translation failed
            translationProgressService.updateLanguageProgress(sessionId, language.languageid, 'failed', translationError.message);
  ...
  async updatePost(postid, updateData, userid) {
    ...
    const { title, content, languageid, categoryids, status, update_all_translations } = updateData;
    ...
    // OR if explicitly requested via update_all_translations flag
    if (!post.originalid && (update_all_translations || (title !== undefined || content !== undefined))) {
      ...
      try {
        ...
        if (translations.length > 0) {
          ...
          // Start translation progress tracking for updates
          const sessionId = `update_${postid}_${Date.now()}`;
          if (update_all_translations) {
            translationProgressService.startSession(sessionId, translations.length, translations.map(t => ({
              languageid: t.languageid,
              language_name: t.language.language_name,
              locale_code: t.language.locale_code
            })));
          }
          ...
          const updatePromises = translations.map(async (translation) => {
            try {
              // Update progress: start translating this language
              if (update_all_translations) {
                translationProgressService.updateLanguageProgress(sessionId, translation.languageid, 'translating');
              }
              ...
              // Update progress: translation completed
              if (update_all_translations) {
                translationProgressService.updateLanguageProgress(sessionId, translation.languageid, 'completed');
              }
              ...
              // Update progress: translation failed
              if (update_all_translations) {
                translationProgressService.updateLanguageProgress(sessionId, translation.languageid, 'failed', translationError.message);
              }
