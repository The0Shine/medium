...
import { Observable, map, tap, catchError, of, delay, switchMap, throwError } from 'rxjs';
...
export class PostService {
  constructor(
    ...
    private translationProgressService: TranslationProgressService
  ) {}
  ...
  // Create post for all languages with progress tracking
  createPostForAllLanguagesWithProgress(
    postData: CreatePostRequest
  ): Observable<CreatePostResponse> {
    // Get available languages first
    return this.languageService.getAvailableLanguages().pipe(
      switchMap((languages: any[]) => {
        const availableLanguages = languages.filter((lang: any) => lang.status === 1);
        const totalLanguages = availableLanguages.length;

        // Start progress tracking
        this.translationProgressService.startTranslation(totalLanguages, 'create');

        // Create post and track real progress
        return this.createPostWithRealProgress(postData, availableLanguages);
      })
    );
  }

  private createPostWithRealProgress(
    postData: CreatePostRequest,
    languages: any[]
  ): Observable<CreatePostResponse> {
    const postDataWithFlag = {
    ...
    // Start the actual post creation
    const createPost$ = this.createPost(postDataWithFlag);

    // Simulate progress updates (since backend doesn't send real-time updates yet)
    const progressUpdates$ = this.simulateTranslationProgress(languages);

    // Combine progress simulation with actual post creation
    return progressUpdates$.pipe(
      switchMap(() => createPost$),
      tap(() => {
        // Mark translation as complete
        this.translationProgressService.finishTranslation(true);
      }),
      catchError(error => {
        this.translationProgressService.finishTranslation(false);
        return throwError(() => error);
      })
    );
  }

  private simulateTranslationProgress(languages: any[]): Observable<void> {
    let currentIndex = 0;

    const updateProgress = (): Observable<void> => {
      if (currentIndex >= languages.length) {
        return of(void 0);
      }

      const currentLang = languages[currentIndex];
      const langName = currentLang.language_name || currentLang.name;

      // Update progress
      this.translationProgressService.updateProgress(
        langName,
        currentIndex + 1,
        `Translating to ${langName}...`
      );

      // Simulate translation time (1-3 seconds per language)
      const translationTime = Math.random() * 2000 + 1000;

      return of(void 0).pipe(
        delay(translationTime),
        tap(() => {
          // Mark language as completed
          this.translationProgressService.completeLanguage(langName);
          currentIndex++;
        }),
        switchMap(() => updateProgress())
      );
    };
  ...
  private simulateTranslationUpdateProgress(languages: any[]): Observable<void> {
    let currentIndex = 0;
    ...
    const updateProgress = (): Observable<void> => {
      ...
      this.translationProgressService.updateProgress(
        langName,
        currentIndex + 1,
        `Updating translation for ${langName}...`
      );

      // Simulate translation update time (1-2.5 seconds per language)
      const translationTime = Math.random() * 1500 + 1000;
  ...
  // Update post for all languages with progress tracking
  updatePostForAllLanguagesWithProgress(
    postId: number,
    ...
    return this.languageService.getAvailableLanguages().pipe(
      switchMap((languages: any[]) => {
        ...
        // Start progress tracking
        this.translationProgressService.startTranslation(totalLanguages, 'update');

        // Update post and track real progress
        return this.updatePostWithRealProgress(postId, postData, availableLanguages);
      })
    );
  }

  private updatePostWithRealProgress(
    postId: number,
    postData: UpdatePostRequest,
    languages: any[]
  ): Observable<Post> {
    const postDataWithFlag = {
      ...postData,
      update_all_translations: true,
    };

    // Start the actual post update
    const updatePost$ = this.updatePost(postId, postDataWithFlag);

    // Simulate progress updates for translation updates
    const progressUpdates$ = this.simulateTranslationUpdateProgress(languages);

    // Combine progress simulation with actual post update
    return progressUpdates$.pipe(
      switchMap(() => updatePost$),
      tap(() => {
        // Mark translation update as complete
        this.translationProgressService.finishTranslation(true);
      }),
      catchError(error => {
        this.translationProgressService.finishTranslation(false);
