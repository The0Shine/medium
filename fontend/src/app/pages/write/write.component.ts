...
import { TranslationProgressComponent } from '../../components/translation-progress/translation-progress.component';
...
@Component({
  selector: 'app-write',
  standalone: true,
  imports: [CommonModule, FormsModule, TranslationProgressComponent],
...
export class WriteComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  toggleCategory(categoryId: number) {
    ...
    if (index > -1) {
      ...
      if (this.selectedCategoryIds.length < 5) {
        this.selectedCategoryIds.push(categoryId);
        // Scroll to the newly selected category tab
        setTimeout(() => this.scrollToActiveCategory(categoryId), 100);
      }
    }
  ...
  private scrollToActiveCategory(categoryId: number) {
    ...
    if (categoryElement && scrollContainer) {
      ...
      if (isOutsideLeft || isOutsideRight) {
        ...
        const scrollLeft = (categoryElement as HTMLElement).offsetLeft - (scrollContainer.clientWidth / 2) + ((categoryElement as HTMLElement).clientWidth / 2);
  ...
  async publishPost() {
    ...
    try {
      ...
      if (this.isEditMode && this.editPostId) {
        ...
        this.postService.updatePostForAllLanguagesWithProgress(+this.editPostId, updateData).subscribe({
          next: (post) => {
            this.isSaving = false;
            this.closePublishModal();
            this.toastService.success('Post updated and translations refreshed successfully!');
        ...
        this.postService.createPostForAllLanguagesWithProgress(postData).subscribe({
          next: (post) => {
            this.isSaving = false;
            this.closePublishModal();
            this.toastService.success('Post saved and translated successfully!');
  ...
  async quickSave() {
    ...
    try {
      ...
      this.postService.updatePostForAllLanguagesWithProgress(+this.editPostId, updateData).subscribe({
        next: () => {
          this.isSaving = false;
          this.toastService.success('Changes saved and translations updated!');
