...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  private initializeComponent() {
    ...
    // Load categories and handle any pending URL params
    this.loadCategories().then(() => {
      // Re-apply URL params if categories just loaded
      const currentParams = this.route.snapshot.queryParams;
      if (currentParams['category']) {
        this.applyCategoryFromUrl(currentParams['category']);
      }
    });
  ...
  private handleCategoryFromUrl(categoryId: string) {
    ...
    // Apply category state immediately if categories are loaded
    this.applyCategoryFromUrl(categoryId);
  }

  private applyCategoryFromUrl(categoryId: string) {
    const numCategoryId = Number(categoryId);
    console.log('🔍 Applying category from URL:', categoryId);
    console.log('📋 Available categories:', this.categories.map(c => ({ id: c.categoryid, label: c.label })));

    // Check if we have real categories loaded
    const hasRealCategories = this.categories.some(c => typeof c.categoryid === 'number' && c.categoryid > 0);

    if (!hasRealCategories) {
      console.log('⚠️ Real categories not loaded yet, will retry after load');
      return;
    }
    ...
    const targetCategory = this.categories.find(c => c.categoryid === numCategoryId);
    if (targetCategory) {
      targetCategory.active = true;
      console.log('✅ Category activated:', targetCategory.label);

      // Scroll to active tab
      setTimeout(() => {
        this.scrollToActiveTab(numCategoryId);
      }, 100);
    } else {
      console.log('❌ Category not found:', numCategoryId);
      console.log('Available category IDs:', this.categories.map(c => c.categoryid));
    }
    ...
    return new Promise((resolve, reject) => {
      this.categoryService.getCategories().subscribe({
        next: (categories) => {
          ...
          resolve(); // Resolve promise when categories are loaded
        },
        error: (error) => {
  ...
  selectTab(categoryId: string | number) {
    ...
    // Scroll to active tab
    setTimeout(() => this.scrollToActiveTab(categoryId), 100);
  ...
  private scrollToActiveTab(categoryId: string | number) {
    console.log('🎯 Scrolling to active tab:', categoryId);
    ...
    // Find the active tab button using data-category-id
    const activeTab = tabsContainer.querySelector(`[data-category-id="${categoryId}"]`) as HTMLElement;

    if (activeTab) {
      console.log('✅ Active tab found, scrolling...');
      ...
      console.log('📏 Tab position:', { isOutsideLeft, isOutsideRight });
      ...
      if (isOutsideLeft || isOutsideRight) {
        ...
        console.log('ℹ️ Tab already visible, no scroll needed');
      }
    } else {
      console.log('❌ Active tab not found for category:', categoryId);
      console.log('Available tabs:', Array.from(tabsContainer.children).map((tab: any) => tab.getAttribute('data-category-id')));
    }
  }
